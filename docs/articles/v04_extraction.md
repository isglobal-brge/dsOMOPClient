...
| survival | episode-grain survival, named competing risks, recurrent events or counting-process risk sets; first/last/all event policies as allowed by the selected format; cohort, observation-period, death and administrative censoring |
...
OMOP event primary key to break same-date ties deterministically. Named
person-level OHDSI estimands deliberately reduce recurrent index
episodes to one deterministic episode per person and pre-aggregate
recurrent outcomes; this prevents joins from multiplying person-time
while still counting repeated outcome events. General long, episode,
recurrent-event and counting-process outputs do not perform that
person-level reduction. Longitudinal windows, bins, washout and risk
intervals currently use calendar days, consistent with required OMOP
`*_date` fields and the federated date contract. Optional `*_datetime`
values do not change ordering or create sub-day risk sets. The source
primary key stays internal, so repeated attachments are not linkable
across episodes through a released high-cardinality event token.
...
`temporalCovariates`, `covariateRef`, `timeRef`, and `personRef`. Every
cohort episode must have exactly one OMOP `observation_period` covering
its index date. Only bins intersecting that period appear in
`personPeriods`; `startDay`/`endDay` remain the requested bin limits,
while `observationStartDay`, `observationEndDay`, and inclusive
`daysObserved` describe the observed part. A missing sparse tuple means
zero only when its `(rowId, timeId)` roster row exists. The descriptive
panel intentionally does not treat cohort end or death as
observation/censoring. Use survival or counting-process output for
explicit time-at-risk, washout, tie, and censoring policies. Arbitrary
multi-state transition models remain outside the reviewed contracts.
Multi-table long recipes split into one output per table rather than
...
(`dsomop.max_temporal_bins`). Longitudinal first/last/nearest selection
retains at most 100 events per episode/source group
(`dsomop.max_events_per_group`). Filter trees are capped at depth 32,
1,024 nodes and 10,000 literal values, and a plan at 100 outputs. These
are controller-configurable memory/CPU safeguards rather than disclosure
...
missing/suppressed levels. Recoding is representation-only: every
locally observed level must already pass the server’s
person-support/cardinality gate and must be covered by the shared
contract. Otherwise the complete recoding call is a silent no-op. This
keeps the original values intact and prevents a partial factor contract
from being used as a category-presence probe; safe levels seen only at
another site can still be retained locally as empty factor levels.
...
Before assignment, the client verifies that the exact namespace owned by
the requested outputs is fresh and refuses to start if any requested
destination already exists. Every expected component must then land on
every node. A node failure, incomplete composite output or
factor-harmonization failure removes and verifies all DataSHIELD-visible
symbols created by the attempt. This is symbol-level all-or-none
cleanup; pre-existing symbols are never overwritten.
...
the server uses its CSV fallback). Long event and interval outputs, plus
SQL-first temporal/person-period components, are fetched and sanitised
in bounded chunks into row groups of one Parquet file, so the complete
result is never materialised in the R session. Formats that require an
R-side transformation are materialised first and only then staged.
