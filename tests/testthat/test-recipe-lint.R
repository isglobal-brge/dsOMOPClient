...
test_that("no_compile: output targets a non-base population", {
  r <- omop_recipe()
  r <- recipe_add_population(r, omop_population(id = "f", parent_id = "base",
         filters = list(omop_filter_sex("F"))))
  r <- recipe_add_variable(r, omop_variable_sex())
  r <- recipe_add_output(r, omop_output(name = "w", type = "wide",
                                        population_id = "f"))
  L <- recipe_lint(r)
...
test_that("empty_output: output resolves to zero variables", {
  r <- omop_recipe()
  r <- recipe_add_variable(r, omop_variable_sex(name = "sex"))
...
test_that("concept_zero: unmapped concept on a non-derived format", {
  r <- omop_recipe()
  r <- recipe_add_variable(r, omop_variable(name = "z", table = "measurement",
...
test_that("concept_unnamed: concept_id present but concept_name missing", {
  r <- omop_recipe()
  r <- recipe_add_variable(r, omop_variable(name = "u", table = "measurement",
...
test_that("narrow_filter: age_range width < 5 years", {
  r <- omop_recipe()
  r <- recipe_add_filter(r, omop_filter_age(min = 40, max = 43))
  r <- recipe_add_variable(r, omop_variable_sex())
  r <- recipe_add_output(r, omop_output(name = "w", type = "wide"))
  L <- recipe_lint(r)
...
test_that("narrow_filter: date_range width < 30 days", {
  r <- omop_recipe()
  r <- recipe_add_population(r, omop_population(id = "base",
         label = "All Persons",
...
test_that("highcard_factor: raw _concept_id column with factor_concepts=TRUE", {
  ...
  r <- recipe_add_variable(r, omop_variable(name = "cc",
...
test_that("window_inverted: time_window start > end", {
  r <- omop_recipe()
  r <- recipe_add_variable(r, omop_variable(name = "w", table = "measurement",
...
test_that("long_split: long output spanning >1 table", {
  r <- omop_recipe()
  r <- recipe_add_variable(r, omop_variable(name = "a",
         table = "condition_occurrence", concept_id = 1, concept_name = "A",
         column = "x", format = "raw"))
  r <- recipe_add_variable(r, omop_variable(name = "b", table = "measurement",
         concept_id = 2, concept_name = "B", column = "y", format = "raw"))
  r <- recipe_add_output(r, omop_output(name = "evt", type = "long"))
  L <- recipe_lint(r)
...
test_that("no_value_source: aggregate format without a value_source", {
  r <- omop_recipe()
  r <- recipe_add_variable(r, omop_variable(name = "m", table = "measurement",
