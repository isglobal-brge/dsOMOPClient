...
Furthermore, support is provided for server-side functions that identify when a table obtained directly from the OMOP CDM database remains unaltered (since being integrated into the DataSHIELD workflow, the table will have the class `omop.cdm.table` in addition to `data.frame`. However, if it is modified in terms of its original composition, it will lose the `omop.cdm.table` class). This class check can be utilized from the server side to streamline the process of enabling methods for aggregating longitudinal data in server-side packages, making the overall data handling process more efficient.
...
Now this information is ready to be used within the DataSHIELD environment for any analysis or data manipulation operation that we want to perform. In the next article, we will see how to use the built-in functions of DataSHIELD's `dsBaseClient` package to combine it with other tables.
