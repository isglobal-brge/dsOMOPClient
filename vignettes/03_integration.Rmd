...
For more information about `dsOMOPHelper` and how it can streamline the process of working with OMOP CDM databases in DataSHIELD, please visit [its GitHub repository](https://github.com/dsOMOPHelper).
...
ds.merge(
  ...
  suffixes = c(".x", ".y"),
  no.dups = TRUE,
  newobj = "person_female_measurements",
  datasources = conns
)
```
...
ds.summary("person_female_measurements")
ds.summary("person_female_measurements$gender_concept_id")
```

This is how we can use DataSHIELD's standard methods to subset tables obtained from OMOP CDM databases. We could also apply more filters to the tables by repeating the process with additional filters and the combined table.
